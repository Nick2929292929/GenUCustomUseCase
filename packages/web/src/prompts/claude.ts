...
${params.context}
  ...
  generateTwoTextPagePrompt(params: GenerateTwoTextParams): string{
    return `<input></input>の情報から<作成する文書の形式></作成する文書の形式>で与える指示に従って、指示された形式の文章のみを出力してください。それ以外の文言は一切出力してはいけません。例外はありません。
出力は<output></output>のxmlタグで囲んでください。
<input>
${params.information}
...
${params.context}
  ...
  generateMailPrompt(params: GenerateMailParams): string {
    return `あなたは株式会社DTSのクラウドソリューションの営業担当です。以下の<Yoken></Yoken>で与える指示に従って、以下で指定する追加の資料を参考に顧客へのメールを出力してください。それ以外の文言は一切出力してはいけません。例外はありません。
            出力は<output></output>のxmlタグで囲んでください。
            ${params.service}の値がAWS DevOps導入支援サービス、${params.mailtype}の値が紹介メールの場合は<input></input>内の資料を基にメールを作成してください、<input2></input2>、<input3></input3>、<input4></input4>の内容は一切参考にしてはいけません、例外はありません。
            ${params.service}の値が生成AIPoC支援サービス、${params.mailtype}の値が紹介メールの場合は<input2></input2>内の資料を基にメールを作成してください、<input1></input1>、<input3></input3>、<input4></input4>の内容は一切参考にしてはいけません、例外はありません。
            ${params.service}の値がAWS DevOps導入支援サービス、${params.mailtype}の値がセミナー案内メールの場合は<input3></input3>内の資料を基にメールを作成してください、<input1></input1>、<input2></input2>、<input4></input4>の内容は一切参考にしてはいけません、例外はありません。
            ${params.service}の値が生成AIPoC支援サービス、${params.mailtype}の値がセミナー案内メールの場合は<input4></input4>内の資料を基にメールを作成してください<input1></input1>、<input2></input2>、<input3></input3>の内容は一切参考にしてはいけません、例外はありません。
            
            <Yoken>
             ・メールの種類:  このメールは${params.service}に関する ${params.mailtype}です。
             ・サービス名:このメールで扱うサービスは${params.service}です。
             ・メールで伝えたいこと: ${params.transmission}      
             ・本文の言語: このメールの件名と本文を${params.writingstyle}で記述してください。
             ・文体: このメールの本文は${params.selected}な文体で記述してください。
             ・メール本文の文字数: このメールの本文は${params.wordcount}単語以下で記述してください。
             ・メール本文の要件: ${params.requirement}
             ・メールの件名の候補の作成数: 本文の前にこのメールの件名を${params.subjectcount}個記述してください。
             ・メールの件名の要件: このメールの件名は${params.context}
            </Yoken>
             
             <input>
             - AWS生成AI PoC支援サービス
              - AWS生成AI PoC支援サービス概要
                - お客様の生成AIのPoCを支援するサービス。様々な形でPoC支援をご提供します。
                - 多機能な生成AIのPoC環境をセキュアに提供するサービスです。 PoC環境の運用保守/セキュリティ管理をDTSにお任せいただくことで、お客様は生成AIのテストに専念することができます。生成AI PoCアプリケーションの画面から、様々な機能を選択するだけで、各機能をご利用いただけます。 画面上で、使用する基盤モデルも選択可能なため、基盤モデルの比較・評価が簡単に行えます。
              - こんな課題はございませんか
                - 以下のような課題を抱えているお客さまに対して非常に効果的なサービスです。
                  - ・生成AIの効果的なユースケースを探している
                  - ・生成AIのユースケースのアイディアはあるが、まずは実現可否をテストしたい
                  - ・セキュリティやコンプライアンス面で不安がある
                  - ・PoC環境をすぐに導入したい
                  - ・PoC環境を運用管理する人材が不足している
                  - ・生成AIのテストの進め方がわからない
              - 3つの特長
                - POINT01：生成AIのセキュアなPoC環境を速やかに提供するマネージドサービス
                  - DTSの実績・ノウハウとAWS Well-Architected Frameworkに準拠した、セキュアなPoC環境をマネージドで提供します。運用・監視・保守・障害対応、セキュリティ・コスト監視をDTSにお任せいただくことで、お客様は生成AIの様々なユースケースや活用アイディアのテストにご専念いただけます。※マネージド
                - POINT02：様々なユースケースに対応した安全な生成アプリ
                  - 生成AIアプリケーションは、AWSが開発した"Generative AI Use Cases JP"（略称 GenU）です。GenUは、生成AIの豊富なユースケースに対応しています。（ユースケース例）チャット、文章生成、要約、校正、翻訳、音声認識、ブログ記事作成 、議事録作成、画像生成、RAGチャットなど ※マネージド、デプロイ支援
                - POINT03：PoC に役立つノウハウもご提供可能
                  - 生成AIのPoCが初めてのお客様に向けて、生成AIの基本用語、基盤モデル、RAGなどPoCに役立つノウハウ情報をご提供いたします。※トレーニング
              - AWS 生成AI PoC 支援サービス詳細
                - サービス詳細は下表のとおりです。
                  - マネージドメニュー
                    - 提供内容
                      -・環境の提供
                      -・運用保守
                      -・問い合わせ対応
                    - 説明：お客様専用の生成AI PoC環境の構築、運用保守、サポートをDTSがマネージドサービスで提供します。
                  - デプロイ支援メニュー
                    - 提供内容
                      -・環境構築支援
                      -・問い合わせ対応
                    - 説明：お客様が利用中のAWS環境に、生成AI PoC環境導入をご希望の場合、DTSが構築支援いたします。
                  - トレーニングメニュー
                    - 提供内容
                      -・生成AI PoCトレーニング
                    - 説明：生成AIの入門知識とPoCに役立つノウハウのトレーニングをWebで提供します。
              - AWS 生成AI PoC 支援サービスのユースケース毎の特徴例
                - ユースケース毎の特徴例
                  - テキスト系
                    -チャット：マルチモーダルの基盤モデルから、画像を使った⽣成も可能
                    -文章生成：⻑い⽂章を⼈間の指⽰通りに再構成
                    -要約：読む価値があるかを数秒で判断する
                    -校正：誤った⽂章を指摘してもらい、意図どおりの⽂章を作成する
                    -翻訳：⻑い⽂章も⼀度に正確な翻訳
                    -RAGチャット：保存したオリジナル文書を、RAGの機能で検索・参照を行い回答を⽣成
                  - 画像系
                      -・画像生成：テキスト⽣成モデルを使って⽇本語でプロンプトを⽣成し、⾼品質な画像を素早く⽣成
                - 補足説明
                  - PoC環境は、AWSがAmazon Bedrockで提供する多数の基盤モデルに対応しています。PoCから複数の基盤モデルを評価し、最適なモデルをご選択いただくことが可能です。
              - AWS生成AI PoC 支援サービスの流れ
                - STEP 1：お問い合わせ：ホームページの問合せフォームにご連絡いただきます。
                - STEP 2：ヒアリング：DTS担当よりお客様のご要件をご確認いたします。
                - STEP 3：ご提案・お見積り：お客様のご要件に沿ったメニューをご提案いたします。
                - STEP 4：申込書の記載：申込書に必要事項をご記載頂き、メールにてご送付頂きます。
                - STEP 5：ご契約：申込書を確認後、契約を締結いたします。
            - サービス紹介ホームページ
              - タイトル：AWS生成AI PoC支援サービス
              - URL：https://dts-digital.jp/cloud/service/aws_generative_ai/
             </input>

             <input2>
             - AWS DevOps導入支援サービス
              - AWS DevOps導入支援サービス概要
                - お客様のビジネス環境に合わせたサービスファクトリーを実現
              ...
              - AWS DevOps導入支援サービスとDepOpsガイダンスの対応関係
                - [例]AWS DevOps導入支援サービス詳細
                  - [例]DepOpsガイダンス
                    - [例]DepOpsガイダンス詳細
                - 組織改革／教育支援
                  - 組織導入
                ...
                - オブザーバビリティ導入支援
                  - 可観測性
                    - 可視化／モニタリング
              - サービスファクトリーとは
                - クラウド・ネイティブを中心とする多様なテクノロジーや方法論を包括し、アプリケーションやインフラをサービスとして提供するためのフレームワーク
              - サービスファクトリーに含まれるテクノロジーや方法論
                - ・クラウドネイティブ
                - ・DevOps
              ...
              - こんな課題はございませんか
                - 以下のような課題を抱えているお客さまに対して非常に効果的なサービスです。
                  ...
                  - ・品質の担保と開発スピードの両立が難しい
              - 3つの特長
                - POINT01：サービスファクトリとして包括的に導入可能
                  - DevOps導入支援、DevSecOps導入支援、オブザーバビリティ導入支援等の個別導入ではなく、サービスファクトリー包括的な支援が可能です。各メニューを包括的に支援することで、お客様に最適なサービスファクトリ環境をトータルで支援します。
                - POINT02：自社に合ったサービスファクトリー環境を利用できる柔軟性
                  - 既存のツールを導入済のお客様に関しては、不足している部分のみの導入も可能。お客様による取捨選択の柔軟性を備え、環境にあった柔軟な支援を行います。
                - POINT03：ユーザー体験の可視化で課題を早期発見
                  - サービスの状態、ユーザー体験を可視化。ユーザー目線のサービスの状態や動向からユーザーの満足度の低下を事前にリアルタイムに観測できるようになります。
              - AWS DevOps導入支援サービス詳細
                - サービス詳細は下表のとおりです。
                  - 組織改革／教育支援
                    - 組織全体の改革や従業員のスキルアップに重点を置き、企業がDevOpsの原則を組織の文化に根付かせ、効率的なソフトウェア開発と運用プロセスを実現することを支援します。 
                  - アプリケーション開発支援
                    - お客様のプロジェクト特性やニーズに合わせた開発ライフサイクル、開発ガイドライン、運用プロセスの最適化を支援します。
                  - CI/CD導入支援
                    - アセスメント支援：お客様の要件をもとにCI/CDの対象および必要な機能を定義し、導入後の運用イメージ、活用方法をご提案いたします。
                    - CI/CDツール導入支援：CI/CDツールを導入し、アプリやIaCのビルドやテスト、デプロイの自動化、テストや構文チェック結果のレポート出力など、インテグレーションとデリバリーをより安心・安全に省力化ができる環境の構築を支援します。
                    - CI/CD運用支援：CI/CD機構を利用していく中での不明点、疑問、その他改善要望などの相談窓口を用意し、環境ご提供後も相談受付や運用支援を行います。
                  - DevSecOps導入支援
                    - アセスメント支援：現状の運用についてヒアリングして現状分析。自動化するセキュリティテストを定義し、導入後の運用イメージ、活用方法をご提案します。
                    - セキュリティツール導入支援：CI/CDパイプラインに統合可能な脆弱性診断ツールを導入し、パイプライン内で自動化。”シフトレフト”思想 に基づき、アプリケーション開発の早期段階から脆弱性の発見および対策が可能な環境の構築を支援します。
                  - オブザーバビリティ導入支援
                    - アセスメント支援：現状の運用についてヒアリングして現状分析。モニタリング対象を再定義し、オブザーバビリティツール導入後の運用イメージ、活用方法をご提案します。
                    - オブザーバビリティツール導入：お客様に適したオブザーバビリティツールを導入し、サービスの状況／状態をモニタリングできる環境を構築します。
                    - オブザーバビリティ運用支援：運用の結果をもとに、モニタリング項目の追加や変更、チューニングなどオブザーバビリティツールの状態を最適に保つための技術支援を行います。
            - サービス紹介ホームページ
              - タイトル：AWS DevOps導入支援サービス
              - URL：https://dts-digital.jp/cloud/service/aws_devops/
             </input2>


             <input3>
             - セミナー:AWS生成AIPoC支援サービス
              - サービス名：AWS生成AIPoC支援サービス
              - タイトル：生成AI活用に必要な、自社業務で活用したい想定ユースケースの検証を解説 〜AWS・GenUによる、生成AIのPoC環境をご紹介〜
              - 開催日：2024年8月20日
              - セミナー形式：Webセミナー
               -ツール：Zoom ※URLは直前にメールにてご連絡いたします。 
              - お客様の課題
            ...
            - サービス紹介ホームページ
             ...
             <input4>
             - セミナー:AWS DevOps導入支援サービス
              - サービス名：AWS DevOps導入支援サービス
              - タイトル：クラウドシフトを成功に導くためのDevOpsアプローチ ～導入事例からみるDevOps導入時のポイント～
              - 開催日：2024年9月25日
              - セミナー形式：Webセミナー
                -ツール：Zoom ※URLは直前にメールにてご連絡いたします。 
              - 背景
                - 急速なクラウドシフトにより、高まるDevOpsの重要性
                  - 近年、デジタルトランスフォーメーション（DX）の加速やリモートワークの普及により、急速なクラウドシフトが進む中、企業のビジネス環境が急速に変化しています。このため、変化する市場や顧客ニーズに迅速に対応し、価値あるサービスをスピーディに提供する必要性が高まっています。こうした状況下で、クラウドネイティブなアプローチを取り入れたDevOpsの重要性がますます増しています。DevOpsを導入することで、品質の向上や自動化が進み、手作業ミスの削減、リリース頻度の向上、開発サイクルの短縮といった多くのメリットが実現できます。
                - DevOps導入における課題とは
            ...
            開発チームと運用チームの連携やスキルギャップといった組織的な課題の解決、CI/CDツールやオブザーバビリティツールの適切な選定、既存システムとのスムーズな統合、そしてDevSecOps（セキュリティを組み込んだDevOps）のアプローチが不可欠です。
              - セミナー内容
                - DevOps導入のつまづきポイントとその解決策
                  - 本セミナーでは、最新のクラウドトレンドや活用事例を交え、これからDevOpsの導入を検討している方を対象に、具体的なユースケースをもとに、導入時の課題や期待される効果について解説します。アマゾンウェブサービスジャパンからは、DevOpsを含む最新のクラウドネイティブのトレンドを、株式会社DTSからは、活用事例や「DevOps導入支援サービス」をご紹介します。
                - 次のような方にオススメ
                  - ・DevOpsの導入を計画中の方
                  - ・DevOpsの導入プロセスを知りたい方
                ...
                - プログラム
                  ...
                  - ・10:10～10:30 「DevOpsを含めた最新のクラウドネイティブのトレンドや活用事例」(アマゾンウェブサービスジャパン)
                  - ・10:30～10:45 「導入事例からみるDevOps導入時のポイント」（DTS）
                  - ・10:45～11:00 質疑応答
                - 主催
                  -株式会社DTS（プライバシー・ポリシー）
              - URL：https://majisemi.com/e/c/dts-20240925
            - サービス紹介ホームページ
              - タイトル：AWS DevOps導入支援サービス
              - URL：https://dts-digital.jp/cloud/service/aws_devops/
             </input4>
